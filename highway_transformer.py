...
attention_network_kwargs = dict(
    in_size=5*15,
    embedding_layer_kwargs={"in_size": 7,
                            "layer_sizes": [64, 64], "reshape": False},
...
def display_vehicles_attention(agent_surface, sim_surface, env, model, min_attention=0.01):
    ...
    for head in range(list(v_attention.values())[0].shape[0]):
        ...
        for vehicle, attention in v_attention.items():
            ...
            desat = np.clip(
                lmap(attention[head], (0, 0.5), (0.7, 1)), 0.7, 1)
            colors = sns.color_palette("dark", desat=desat)
            color = np.array(colors[(2*head) % (len(colors) - 1)]) * 255
            color = (
                *color, np.clip(lmap(attention[head], (0, 0.5), (100, 200)), 100, 200))
...
def compute_vehicles_attention(env, model):
    ...
    attention = model.policy.features_extractor.extractor.get_attention_matrix(
        obs_t)
    attention = attention.squeeze(0).squeeze(1).detach().cpu().numpy()
    ego, others, mask = model.policy.features_extractor.extractor.split_input(
        obs_t)
    mask = mask.squeeze()
    v_attention = {}
    ...
    for v_index in range(obs.shape[0]):
        ...
        for feature in ["x", "y"]:
            ...
            v_feature = lmap(v_feature, [-1, 1],
                             obs_type.features_range[feature])
        ...
        vehicle = min(env.unwrapped.road.vehicles,
